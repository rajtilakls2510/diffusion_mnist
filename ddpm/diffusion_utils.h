...
inline torch::Tensor linear_schedule(int timesteps, float beta_start = 1e-4,
                                     float beta_end = 1e-2) {
...
inline torch::Tensor cosine_beta_schedule(int timesteps) {
  ...
  torch::Tensor betas = 1 - (alphas_cumprod.index({Slice(1, None)}) /
                             alphas_cumprod.index({Slice(0, -1)}));
  return torch::clip(betas, 0, 0.999);
}

inline torch::Tensor extract(torch::Tensor a, torch::Tensor t,
                             const at::IntArrayRef &x_shape) {
  int64_t b = t.size(0);
