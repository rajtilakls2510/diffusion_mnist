...
Loader::Loader(std::string data_path, int batch_size)
  ...
  std::tie(train_images, train_labels) =
      read_images_labels(train_images_path, train_labels_path);
  std::tie(test_images, test_labels) =
      read_images_labels(test_images_path, test_labels_path);
...
Loader::read_images_labels(std::string &images_path, std::string &labels_path) {
  ...
  torch::Tensor labels_tensor =
      torch::from_blob(labels.data(), {static_cast<long>(num_items)},
                       torch::kUInt8)
          .clone();
  ...
  torch::Tensor images_tensor =
      torch::from_blob(images.data(),
                       {static_cast<long>(num_images), 1,
                        static_cast<long>(rows), static_cast<long>(cols)},
                       torch::kUInt8)
          .clone();

  return {images_tensor.to(torch::kFloat32).div(255.0),
          labels_tensor.to(torch::kLong)};
}
...
// ======================= Visualization and verification code (TO BE REMOVED
// LATER)
// ================
...
cv::Mat Loader::tensor_to_cv_mat(torch::Tensor image_tensor) {
  ...
  if (image_tensor.sizes().size() == 3 && image_tensor.size(0) == 3) {
    ...
    return cv::Mat(image_tensor.size(0), image_tensor.size(1), CV_8UC3,
  ...
  // Step 4: If grayscale [1, H, W], convert to [H, W]
    ...
    return cv::Mat(image_tensor.size(0), image_tensor.size(1), CV_8UC1,
                   image_tensor.data_ptr());
