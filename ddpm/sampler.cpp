...
// Convert a batch of [B,1,28,28] tensors into a single OpenCV image arranged in
// a grid
cv::Mat batch_to_mat(torch::Tensor batch, int scale = 4,
  ...
  int rows =
      (B + images_per_row - 1) / images_per_row; // ceil(B / images_per_row)
  ...
  for (int i = 0; i < B; i++) {
    ...
    cv::Mat resized;
    cv::resize(img, resized, cv::Size(W * scale, H * scale), 0, 0,
               cv::INTER_NEAREST);
...
int main(int argc, char *argv[]) {
  ...
  torch::Tensor alphas_cumprod_prev = torch::cat(
      {torch::ones({1}).to(device), alphas_cumprod.index({Slice(0, -1)})});
  torch::Tensor sqrt_one_minus_alphas_cumprod =
      torch::sqrt(1.0 - alphas_cumprod);
  torch::Tensor beta_tildes =
      ((1 - alphas_cumprod_prev) * betas) / (1.0 - alphas_cumprod);
  ...
  for (int t = max_diffusion_time; t > 0; t--) {
    ...
    torch::Tensor sqrt_alpha_t =
        torch::sqrt(extract(alphas, timesteps - 1, x.sizes()));
    ...
    if (!isWriterOpen) {
      writer.open(video_path, cv::VideoWriter::fourcc('a', 'v', 'c', '1'), fps,
                  canvas.size(),
  ...
  cv::imwrite(image_path,
              batch_to_mat(torch::clamp(x, -1.0, 1.0).add(1).div(2), 10));
