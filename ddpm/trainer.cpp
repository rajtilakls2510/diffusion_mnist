...
Trainer::Trainer(std::string data_path, std::string checkpoint_path,
                 int max_diffusion_time, bool use_cpu)
...
torch::Tensor Trainer::q_sample(torch::Tensor x_start, torch::Tensor t,
                                torch::Tensor noise) {
...
void Trainer::train_step(Batch batch, double *loss) {
  ...
  torch::Tensor t, noise, x_noisy;
  {
    ...
    t = torch::randint(1, max_diffusion_time + 1, {/*batch*/ batch.batch_size},
                       torch::kLong)
            .to(device);
  ...
  for (int epoch = 0; epoch < epochs; epoch++) {
    ...
    while (true) {
      try {
        ...
        n_batch++;
        avg_train_loss =
            avg_train_loss + (1.0 / n_batch) * (loss - avg_train_loss);
    ...
    while (true) {
      try {
        ...
        n_batch++;
        avg_test_loss =
            avg_test_loss + (1.0 / n_batch) * (loss - avg_test_loss);
